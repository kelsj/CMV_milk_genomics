...
# N=142 with all data points to calculate residuals
kyn.meta.resid = metabs %>% select(indID, KYNURENINE, TRYPTOPHAN) %>% mutate(KYN = scale(log(KYNURENINE))) %>%
  left_join(metadata %>% 
              select(indID, shotgun.cmv.status, Center, parity, matage, mat_bmi, mat.white, avg_totalhei, gdm)) %>%
...
ggplot(kyn.meta.resid, aes(x=shotgun.cmv.status,y=KYNresid,color=shotgun.cmv.status)) + 
...
ido.dat = tpm %>% filter(gene_id=="ENSG00000131203") %>% gather(key="indID",value="IDO1",2:222) %>%
  left_join(metadata %>% select(indID, shotgun.cmv.status)) %>% filter(!is.na(shotgun.cmv.status))
my_y_title = bquote("Milk"~italic("IDO1")~"expression (TPM)")

ggplot(ido.dat, aes(y=IDO1,x=shotgun.cmv.status,color=shotgun.cmv.status)) + 
...
ggplot(kyn.meta.resid, aes(x=IDO1,y=KTRresid,color=shotgun.cmv.status)) + geom_smooth(method="lm",se=F) + 
